 ...
 */
_pushkinJspsych["default"].plugins["moving-window"] = function () {
  var plugin = {};
  plugin.info = {
    name: "moving-window",
    parameters: {
      words: {
        type: _pushkinJspsych["default"].plugins.parameterType.STRING,
        "default": undefined
      },
      key: {
        type: _pushkinJspsych["default"].plugins.parameterType.KEYCODE,
        "default": 32
      },
      rate: {
        type: _pushkinJspsych["default"].plugins.parameterType.STRING,
  ...
  plugin.trial = function (display_element, trial) {
    ...
    function create_separate_window(words, position) {
      if (trial.rate.includes('sentence')) {
        ...
        if (trial.rate.includes('word')) {
          ...
          var word_list = target_sentence.split(' ');
          var sentence_pre = sentence_holder.splice(0, n_sentences - 2);
          var sentence_pre2 = sentence_pre.map(function (x) {
            return x + '.';
          });
    ...
    function show_stimulus(position) {
      ...
      _pushkinJspsych["default"].pluginAPI.getKeyboardResponse({
    ...
    function end_trial() {
      ...
      _pushkinJspsych["default"].finishTrial(trial_data);
